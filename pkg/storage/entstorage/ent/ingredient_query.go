...
//		GroupBy(ingredient.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Ingredient.Query().
//		Select(ingredient.FieldCreateTime).
//		Scan(ctx, &v)
...
func (iq *IngredientQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ingredient, error) {
	...
	if query := iq.withRecipe; query != nil {
		if err := iq.loadRecipe(ctx, query, nodes, nil,
			func(n *Ingredient, e *Recipe) { n.Edges.Recipe = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withProduct; query != nil {
		if err := iq.loadProduct(ctx, query, nodes, nil,
			func(n *Ingredient, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *IngredientQuery) loadRecipe(ctx context.Context, query *RecipeQuery, nodes []*Ingredient, init func(*Ingredient), assign func(*Ingredient, *Recipe)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ingredient)
	for i := range nodes {
		fk := nodes[i].RecipeID
...
func (iq *IngredientQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Ingredient, init func(*Ingredient), assign func(*Ingredient, *Product)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ingredient)
	for i := range nodes {
...
func (iq *IngredientQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(ingredient.Table, ingredient.Columns, sqlgraph.NewFieldSpec(ingredient.FieldID, field.TypeUUID))
	_spec.From = iq.sql
	...
	if fields := iq.ctx.Fields; len(fields) > 0 {
		...
		if iq.withRecipe != nil {
			_spec.Node.AddColumnOnce(ingredient.FieldRecipeID)
		}
		if iq.withProduct != nil {
			_spec.Node.AddColumnOnce(ingredient.FieldProductID)
		}
	}
