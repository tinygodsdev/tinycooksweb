...
func (f InstructionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.InstructionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.InstructionMutation", m)
}

// The NutritionFunc type is an adapter to allow the use of ordinary
// function as Nutrition mutator.
type NutritionFunc func(context.Context, *ent.NutritionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NutritionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NutritionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.NutritionMutation", m)
}
