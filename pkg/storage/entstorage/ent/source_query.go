...
func (sq *SourceQuery) QueryRecipe() *RecipeQuery {
	query := (&RecipeClient{config: sq.config}).Query()
...
func (sq *SourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Source, error) {
	...
	if query := sq.withRecipe; query != nil {
		if err := sq.loadRecipe(ctx, query, nodes, nil,
			func(n *Source, e *Recipe) { n.Edges.Recipe = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SourceQuery) loadRecipe(ctx context.Context, query *RecipeQuery, nodes []*Source, init func(*Source), assign func(*Source, *Recipe)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Source)
	for i := range nodes {
		if nodes[i].recipe_sources == nil {
			continue
		}
		fk := *nodes[i].recipe_sources
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "recipe_sources" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
