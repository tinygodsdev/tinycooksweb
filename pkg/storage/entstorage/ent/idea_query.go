...
func (iq *IdeaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Idea, error) {
	...
	if query := iq.withRecipe; query != nil {
		if err := iq.loadRecipe(ctx, query, nodes, nil,
			func(n *Idea, e *Recipe) { n.Edges.Recipe = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *IdeaQuery) loadRecipe(ctx context.Context, query *RecipeQuery, nodes []*Idea, init func(*Idea), assign func(*Idea, *Recipe)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Idea)
	for i := range nodes {
		if nodes[i].recipe_ideas == nil {
			continue
		}
		fk := *nodes[i].recipe_ideas
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "recipe_ideas" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
