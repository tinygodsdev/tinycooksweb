...
func (eq *EquipmentQuery) QueryRecipes() *RecipeQuery {
	query := (&RecipeClient{config: eq.config}).Query()
...
func (eq *EquipmentQuery) Clone() *EquipmentQuery {
	...
	return &EquipmentQuery{
		...
		withRecipes: eq.withRecipes.Clone(),
...
func (eq *EquipmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Equipment, error) {
	...
	if query := eq.withRecipes; query != nil {
		if err := eq.loadRecipes(ctx, query, nodes,
			func(n *Equipment) { n.Edges.Recipes = []*Recipe{} },
			func(n *Equipment, e *Recipe) { n.Edges.Recipes = append(n.Edges.Recipes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EquipmentQuery) loadRecipes(ctx context.Context, query *RecipeQuery, nodes []*Equipment, init func(*Equipment), assign func(*Equipment, *Recipe)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Equipment)
	nids := make(map[uuid.UUID]map[*Equipment]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(equipment.RecipesTable)
		s.Join(joinT).On(s.C(recipe.FieldID), joinT.C(equipment.RecipesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(equipment.RecipesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(equipment.RecipesPrimaryKey[1]))
