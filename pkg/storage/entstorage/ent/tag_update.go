...
func (tu *TagUpdate) RemoveRecipeIDs(ids ...uuid.UUID) *TagUpdate {
	tu.mutation.RemoveRecipeIDs(ids...)
	return tu
}
...
func (tu *TagUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := tu.mutation.RemovedRecipesIDs(); len(nodes) > 0 && !tu.mutation.RecipesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.RecipesTable,
	...
	if nodes := tu.mutation.RecipesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.RecipesTable,
...
func (tuo *TagUpdateOne) RemoveRecipes(r ...*Recipe) *TagUpdateOne {
	...
	return tuo.RemoveRecipeIDs(ids...)
}
...
func (tuo *TagUpdateOne) sqlSave(ctx context.Context) (_node *Tag, err error) {
	...
	if nodes := tuo.mutation.RemovedRecipesIDs(); len(nodes) > 0 && !tuo.mutation.RecipesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.RecipesTable,
	...
	if nodes := tuo.mutation.RecipesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.RecipesTable,
