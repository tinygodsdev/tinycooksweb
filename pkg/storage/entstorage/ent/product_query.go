...
type ProductQuery struct {
	...
	withRequiredInRecipes *RecipeQuery
...
func (pq *ProductQuery) QueryRequiredInRecipes() *RecipeQuery {
	query := (&RecipeClient{config: pq.config}).Query()
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withRequiredInRecipes: pq.withRequiredInRecipes.Clone(),
		withIngredients:       pq.withIngredients.Clone(),
...
// WithRequiredInRecipes tells the query-builder to eager-load the nodes that are connected to
// the "required_in_recipes" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithRequiredInRecipes(opts ...func(*RecipeQuery)) *ProductQuery {
	query := (&RecipeClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withRequiredInRecipes = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [2]bool{
			pq.withRequiredInRecipes != nil,
			pq.withIngredients != nil,
		}
	)
	...
	if query := pq.withRequiredInRecipes; query != nil {
		if err := pq.loadRequiredInRecipes(ctx, query, nodes,
			func(n *Product) { n.Edges.RequiredInRecipes = []*Recipe{} },
			func(n *Product, e *Recipe) { n.Edges.RequiredInRecipes = append(n.Edges.RequiredInRecipes, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withIngredients; query != nil {
		if err := pq.loadIngredients(ctx, query, nodes,
			func(n *Product) { n.Edges.Ingredients = []*Ingredient{} },
			func(n *Product, e *Ingredient) { n.Edges.Ingredients = append(n.Edges.Ingredients, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadRequiredInRecipes(ctx context.Context, query *RecipeQuery, nodes []*Product, init func(*Product), assign func(*Product, *Recipe)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(product.RequiredInRecipesTable)
		s.Join(joinT).On(s.C(recipe.FieldID), joinT.C(product.RequiredInRecipesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(product.RequiredInRecipesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.RequiredInRecipesPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "required_in_recipes" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (pq *ProductQuery) loadIngredients(ctx context.Context, query *IngredientQuery, nodes []*Product, init func(*Product), assign func(*Product, *Ingredient)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ingredient.FieldProductID)
	}
	query.Where(predicate.Ingredient(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.IngredientsColumn), fks...))
	}))
